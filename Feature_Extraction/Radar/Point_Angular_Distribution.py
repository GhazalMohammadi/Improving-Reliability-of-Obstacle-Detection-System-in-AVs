# RADAR points angulr distribution==============================================================================================
# ===============================================================================================================================
import json
import numpy as np
...
def visualize_last_frame(entry, ego_pose, output_entry, lidar_points_global, angles):
    ...
    radius = 250  # Changed to 250 meters
    label_radius = 270  # Increased for better label placement
    ...
    for dist in [50, 100, 150, 200, 250]:  # Updated distances for 250m radius
    ...
    # Set appropriate axis limits for 250m radius
    ax1.set_xlim(ego_x - 280, ego_x + 280)
    ax1.set_ylim(ego_y - 280, ego_y + 280)
    
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ax1.set_title(f'RADAR Points with Angular Slices (250m radius)\nFrame: {entry["token"][:8]}...\n(Circles are for visualization only - not used in calculations)')
    ax1.legend()
    ...
    ax2.set_title('Angular Distribution of RADAR Points\n(Polar Plot)')
    ax2.grid(True)
    ...
    print(f"Total RADAR Points: {len(lidar_points_global)}")
    ...
    print("The circles at 50m, 100m, 150m, 200m, and 250m are ONLY for visual reference.")
    ...
    print("Distance filtering is NOT applied - all points within RADAR range are included.")
    print(f"{'!'*80}")
...
for entry in lidar_data:
    ...
    output_entry = {
        ...
        "type_of_feature_Extraction":"radarFront_points_sector_distribution_faulty",
...
# Save to output file
output_file = './PATH/radar_points_angular_distribution.json'
...
print("The circles shown in visualization (50m, 100m, 150m, 200m, 250m) are ONLY for visual reference.")
print("They do NOT affect the point distribution calculations.")
print("The algorithm considers ALL RADAR points regardless of distance.")
