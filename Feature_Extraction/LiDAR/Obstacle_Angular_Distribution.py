# LIDAR Obstacle Angular Distribution ====================================================================================
# =======================================================================================================================
import json
import numpy as np
...
def calculate_sector_distribution_for_box(box_corners, ego_translation, sectors, max_distance=100):
    """
    ...
    The sum of all ratios will be 1.0 (if box is completely within 100m range).
    """
    ...
    for i in range(num_samples_per_dim):
        for j in range(num_samples_per_dim):
            ...
            # Only count samples within 100m range
...
def plot_sector_visualization(ego_translation, annotations, sectors, frame_info):
    ...
    # Plot 100m circle
    circle = plt.Circle((0, 0), 100, color='blue', fill=False, linestyle='--', linewidth=1, alpha=0.5, label='100m Range')
    ...
    for i, (start_angle, end_angle) in enumerate(sectors):
        ...
        wedge = Wedge((0, 0), 100, start_angle, end_angle, alpha=0.1, color=color)
        ax1.add_patch(wedge)
        
        # Add sector label at 80m
        mid_angle = (start_angle + end_angle) / 2
        rad = math.radians(mid_angle)
        label_x = 80 * math.cos(rad)
        label_y = 80 * math.sin(rad)
    ...
    # Add coordinate axes
    ax1.arrow(0, 0, 30, 0, head_width=2, head_length=3, fc='green', ec='green', label='Front (0°)')
    ax1.arrow(0, 0, 0, 30, head_width=2, head_length=3, fc='green', ec='green', label='Left (90°)')
    ax1.text(35, 0, '0°', fontsize=10, color='green', weight='bold')
    ax1.text(0, 35, '90°', fontsize=10, color='green', weight='bold')
    ax1.text(-35, 0, '180°', fontsize=10, color='green', weight='bold')
    ax1.text(0, -35, '270°', fontsize=10, color='green', weight='bold')
    ...
    for start_angle, end_angle in sectors:
        ...
        ax2.plot([start_rad, start_rad], [0, 100], 'r-', alpha=0.3, linewidth=0.5)
...
for entry_idx, entry in enumerate(lidar_data):
    ...
    output_entry = {
        ...
        "type_of_feature_Extraction":"lidar_obstacle_sector_distribution_faulty",
...
# Save to output file
output_file = './PATH/lidar_obstacle_angular_distribution.json'
